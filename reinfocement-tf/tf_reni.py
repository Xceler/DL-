...
class PolicyGradientAgent():
    def __init__(self, ALPHA, GAMMA = 0.95, n_actions = 4,
                 ...
                 chkpt_dir = 'reniforcement-tf/checkpoints'):
    ...
    def build_net(self):
        with tf.variable_scope('parameters'):
            ...
            self.G = tf.plcaeholder(tf.float32, shape = [None,], name ='G') 
        ...
        with tf.variable_scope('loss'):
            negative_log_probability = tf.nn.sparse_softmax_cross_entropy_with_logits(
                logits = 13, labels = self.label
            )

            loss = negatvie_log_probability * self.G 
