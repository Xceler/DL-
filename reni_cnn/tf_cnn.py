...
class PolicyGradientAgent(object):
    def __init__(self, ALPHA, GAMMA = 0.95, n_actions = 4, fc1 = 256, 
                 input_shape = (185, 95), channels =1, chkpt_dir ='reni_cnn/checkpoints',
                 gpu = {'GPU' : 1}):
    ...
    def build_net(self):
        ...
        with tf.variable_scope('loss'):
            ...
            self.loss = tf.reduce_mean(negatvie_log_probability * self.G)
    ...
    def choose_action(self, observation):
        observation = np.array(observation).reshape((-1, self.self.input_height,
                                                    self.input_width, 
                                                    self.channels))
